...
#' @param value_pairs a named list of columns and their acceptable range (a vector with a minimum and maximum)
...
expect_range <- function(data, value_pairs, na_accept = FALSE, warn = FALSE){
  ...
  temp <- map(seq_along(value_pairs), function(x){
    ...
    temp_min <- as.numeric(value_pairs[[x]][1])
    temp_max <- as.numeric(value_pairs[[x]][2])
    ...
    if (is.na(temp_max) | is.na(temp_min)){
      stop(str_c(temp_col, ": Missing minimum or maximum value."))
    }
